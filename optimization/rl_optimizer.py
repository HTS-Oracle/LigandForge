...
class RLOptimizer:
    ...
    def _initialize_action_weights(self) -> Dict[str, float]:
        """Initialize action weights for policy gradient method"""
        actions = ['modify', 'extend', 'substitute', 'bioisostere', 'scaffold_hop', 'ring_modification']
        
        # Initialize with equal weights
        initial_weight = 1.0 / len(actions)
        weights = {action: initial_weight for action in actions}
        
        # Add slight bias towards proven actions if available
        # For modify and extend actions which are generally safer
        weights['modify'] *= 1.1
        weights['extend'] *= 1.1
    ...
    def _get_best_action_type(self) -> str:
        """Get the best performing action type based on current weights"""
        if not self.action_weights:
            return 'modify'  # Default fallback
        
        return max(self.action_weights.items(), key=lambda x: x[1])[0]
    
    def _get_current_time(self) -> float:
        """Get current timestamp for optimization history"""
        import time
        return time.time()
    
    def _calculate_population_diversity(self, population: List[Chem.Mol]) -> float:
        ...
        try:
            ...
            # Fallback diversity calculation without RDKit fingerprints
            pass
        except Exception:
            pass
        
        # Simple fallback based on SMILES diversity
        try:
            smiles_set = set()
            for mol in population:
            ...
            return len(smiles_set) / len(population)
        except:
    ...
    def _select_fragment_type(self, action_type: str) -> str:
        """Select appropriate fragment type based on action"""
        fragment_types = {
            'modify': ['substituent', 'functional_group'],
            'extend': ['linker', 'ring', 'chain'],
            'substitute': ['substituent', 'functional_group'],
            'bioisostere': ['bioisostere', 'replacement'],
            'scaffold_hop': ['scaffold', 'core'],
            'ring_modification': ['ring', 'cyclic']
        }
        
        types = fragment_types.get(action_type, ['general'])
    ...
    def get_optimization_statistics(self) -> Dict[str, Any]:
        """
        Public accessor for optimization statistics.
        Returns a stable, serializable dict including high-level metrics,
        current policy info, and a snapshot of the most recent iteration.
        """
        stats = dict(self.optimization_stats)  # copy so callers can't mutate internals
        stats["action_weights"] = dict(self.action_weights)
        stats["exploration_rate"] = float(self.exploration_rate)
        stats["learning_rate"] = float(self.learning_rate)
        stats["total_states"] = len(self.state_history)

        # Add a snapshot from the latest state (if any)
        if self.state_history:
            last = self.state_history[-1]
            try:
                last_avg = float(np.mean(last.current_scores)) if last.current_scores else None
            except Exception:
                last_avg = None

            stats.update({
                "last_iteration": int(last.iteration),
                "last_best_score": float(last.best_score) if last.best_score is not None else None,
                "last_avg_score": last_avg,
                "last_population_size": len(last.current_population) if last.current_population else 0,
                "last_population_diversity": float(last.population_diversity) if last.population_diversity is not None else None,
                "last_convergence_metric": float(last.convergence_metric) if last.convergence_metric is not None else None,
            })
